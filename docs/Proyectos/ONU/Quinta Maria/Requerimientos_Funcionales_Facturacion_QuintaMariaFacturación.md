# Requerimientos Funcionales — Facturación Quinta María (Gestión de CFDIs)
...
| Fecha   | 2026-08-31   |
| Estado  | Definición   |
| Módulo  | Facturación Quinta María — Gestión de CFDIs (Factura, Nota de Crédito, Abono, Anticipo) |
...
**Incluye:**
- Emisión, sustitución y cancelación de CFDIs de tipo Factura, Nota de Crédito, Abono y Anticipo dentro del sistema de Facturación Quinta María.
...
|-------------|-------------|
| Usuario de Facturación (Emisor) | Persona operativa que emite CFDIs de tipo Factura, Nota de Crédito, Abono o Anticipo desde el sistema de Facturación Quinta María (p. ej. usuarios "inmfac", "inmfac02"). |
...
Este documento aplica sobre la entidad **CFDI**, en sus cuatro tipos: **Factura, Nota de Crédito, Abono y Anticipo**, dentro del sistema de Facturación Quinta María.
...
|----|--------|---------|----------|
| [RF-01](#rf-01) | Emisión de CFDI dentro del plazo permitido y timbrado | Facturación Quinta María | Factura, NC, Abono, Anticipo |
| [RF-02](#rf-02) | Emisión de CFDI por sustitución | Facturación Quinta María | Factura, NC, Abono, Anticipo |
| [RF-03](#rf-03) | Emisión de CFDI en diferentes monedas y tipo de cambio | Facturación Quinta María | Factura, NC, Abono, Anticipo |
| [RF-04](#rf-04) | Emisión de Nota de Crédito | Facturación Quinta María | Nota de Crédito |
| [RF-05](#rf-05) | Cancelación general de CFDI (información obligatoria diferenciada: Fiscal vs. No Fiscal) | Facturación Quinta María | Factura, NC, Abono, Anticipo |
| [RF-06](#rf-06) | Cancelación de CFDI emitido para sustitución | Facturación Quinta María | Factura, NC, Abono, Anticipo |
| [RF-07](#rf-07) | Exclusión del motivo "Global" en cancelación de Abonos y Anticipos | Facturación Quinta María | Abono, Anticipo |
| [RF-08](#rf-08) | Cancelación de Factura fiscal | Facturación Quinta María | Factura (fiscal, Timbrada) |
| [RF-09](#rf-09) | Cancelación de Abono fiscal | Facturación Quinta María | Abono (fiscal, Timbrado) |
| [RF-10](#rf-10) | Cancelación de Anticipo fiscal | Facturación Quinta María | Anticipo (fiscal, Timbrado) |
...
## Descripción
El sistema deberá permitir la emisión y timbrado manual de Notas de Crédito únicamente de tipo Diferencia de Precio, Descuento y Descuento por Concepto, exigiendo un permiso especial distinto para cada tipo, validando que la Factura relacionada esté Finalizada y Timbrada, y timbrando siempre con Método de Pago PUE. Los tipos Devolución y Cancelación no estarán disponibles para emisión manual: se generan exclusivamente de forma automática en devoluciones de cliente y en cancelaciones de Factura ya timbrada, respectivamente.

## HU-4.1 — Emitir Nota de Crédito por tipo

Como usuario responsable de la emisión de CFDIs, quiero emitir manualmente una Nota de Crédito del tipo correspondiente (Diferencia de Precio, Descuento o Descuento por Concepto), para ajustar el importe de una Factura conforme a las reglas fiscales aplicables.
...
**RN-4.5** Los tipos de Nota de Crédito disponibles para emisión manual (Diferencia de Precio, Descuento, Descuento por Concepto) requieren un permiso especial distinto entre sí.
...
**RN-4.10** El sistema únicamente permite la emisión y timbrado manual de Notas de Crédito de tipo Diferencia de Precio, Descuento y Descuento por Concepto; los tipos Devolución y Cancelación no estarán disponibles para selección en la emisión manual.
...
Cuando confirma la emisión seleccionando el tipo aplicable (Diferencia de Precio, Descuento o Descuento por Concepto)
...
## HU-4.2 — Restringir la emisión manual de Nota de Crédito a los tipos permitidos

Como usuario responsable de la emisión de CFDIs, quiero que el sistema únicamente me permita seleccionar y timbrar los tipos de Nota de Crédito Diferencia de Precio, Descuento y Descuento por Concepto al emitir manualmente, para evitar que se generen por esta vía Notas de Crédito de tipo Devolución o Cancelación, cuya generación es exclusivamente automática.

### Reglas de negocio

**RN-4.11** El sistema únicamente debe mostrar como opciones de emisión manual los tipos de Nota de Crédito: Diferencia de Precio, Descuento y Descuento por Concepto.

**RN-4.12** Los tipos de Nota de Crédito Devolución y Cancelación no estarán disponibles para selección en la emisión manual; se generan exclusivamente de forma automática conforme a RN-4.9 (devoluciones de cliente) y RN-4.4 (cancelación de Factura), respectivamente.

### Criterios de Aceptación

**CA-4.2.1 — Selección restringida a tipos permitidos**
Dado que el usuario inicia la emisión manual de una Nota de Crédito
Cuando visualiza las opciones de tipo disponibles
Entonces el sistema únicamente muestra los tipos Diferencia de Precio, Descuento y Descuento por Concepto.

**CA-4.2.2 — Bloqueo de emisión manual de tipo Devolución**
Dado que el usuario intenta emitir manualmente una Nota de Crédito de tipo Devolución
Cuando confirma la operación
Entonces el sistema impide la emisión, informando que este tipo solo se genera automáticamente en devoluciones de cliente.

**CA-4.2.3 — Bloqueo de emisión manual de tipo Cancelación**
Dado que el usuario intenta emitir manualmente una Nota de Crédito de tipo Cancelación
Cuando confirma la operación
Entonces el sistema impide la emisión, informando que este tipo solo se genera automáticamente al cancelar una Factura ya timbrada.
...
<a id="rf-12"></a>
# RF-12 — Ajuste a interfaz gráfica: acomodar los campos de la pantalla Detail Factura al acomodo de Inmobiliaria
...
## Objetivo
Uniformar la disposición visual de los campos en la pantalla de detalle de CFDI (Detail Factura) del sistema de Facturación Quinta María, alineándola con el acomodo ya utilizado en la pantalla equivalente del sistema de Facturación Inmobiliaria, para ofrecer una experiencia de captura y consulta consistente entre ambos sistemas.

## Descripción
El sistema deberá reorganizar la disposición (orden y agrupación) de los campos visibles en la pantalla de detalle de CFDI —Factura, Nota de Crédito, Abono y Anticipo—, replicando el acomodo de campos ya utilizado en la pantalla equivalente del sistema de Facturación Inmobiliaria, sin alterar el comportamiento funcional, las validaciones ni las reglas de negocio de emisión y cancelación definidas en los demás requerimientos de este documento (RF-01 a RF-11). El detalle campo por campo del nuevo acomodo (qué campo se reubica, en qué grupo y en qué orden) queda pendiente de confirmación con negocio mediante el mockup o especificación de pantalla correspondiente (ver PA-08).

## HU-12.1 — Visualizar la pantalla Detail Factura con el acomodo de campos de Inmobiliaria

Como usuario de Facturación, quiero que la pantalla de detalle de CFDI (Detail Factura) muestre sus campos con el mismo acomodo (orden y agrupación) que la pantalla equivalente del sistema de Facturación Inmobiliaria, para tener una experiencia de captura y consulta consistente entre ambos sistemas y reducir la curva de aprendizaje al alternar entre ellos.

### Reglas de negocio

**RN-12.1** La pantalla de detalle de CFDI (Detail Factura) del sistema de Facturación Quinta María deberá reorganizar sus campos conforme al acomodo (orden y agrupación) definido en la pantalla equivalente del sistema de Facturación Inmobiliaria.

**RN-12.2** El ajuste de interfaz gráfica no deberá modificar el comportamiento funcional, las validaciones ni las reglas de negocio de emisión y cancelación descritas en los demás requerimientos de este documento (RF-01 a RF-11); únicamente aplica a la disposición visual de los campos.

**RN-12.3** El acomodo de campos deberá aplicarse de manera consistente a los cuatro tipos de comprobante gestionados en la pantalla (Factura, Nota de Crédito, Abono y Anticipo), respetando los campos que sean propios o exclusivos de cada tipo.

### Criterios de Aceptación

**CA-12.1.1 — Acomodo de campos conforme al layout de Inmobiliaria**
Dado que el usuario abre la pantalla de detalle de un CFDI (Factura, Nota de Crédito, Abono o Anticipo) en Facturación Quinta María
Cuando visualiza la pantalla
Entonces el sistema presenta los campos con el mismo orden y agrupación que la pantalla equivalente del sistema de Facturación Inmobiliaria.

**CA-12.1.2 — Sin afectación al comportamiento funcional**
Dado que la pantalla de detalle de CFDI fue actualizada con el nuevo acomodo de campos
Cuando el usuario captura, emite o cancela un CFDI
Entonces el sistema conserva el mismo comportamiento, validaciones y reglas de negocio vigentes antes del ajuste de interfaz.

**CA-12.1.3 — Consistencia del acomodo entre tipos de comprobante**
Dado que el usuario alterna entre las pantallas de detalle de Factura, Nota de Crédito, Abono y Anticipo
Cuando compara la disposición de los campos comunes a los cuatro tipos
Entonces el sistema mantiene el mismo acomodo para dichos campos comunes en los cuatro tipos de comprobante.

**CA-12.1.4 — Campos exclusivos de un tipo de comprobante**
Dado que un campo aplica únicamente a un tipo específico de comprobante (p. ej. un campo exclusivo de Nota de Crédito)
...
| OBJ-4 | RF-04 | HU-4.2 | CA-4.2.1, CA-4.2.2, CA-4.2.3 | Must | Propuesto |
| OBJ-4 | RF-05 | HU-5.1 | CA-5.1.1, CA-5.1.2 | Must | Propuesto |
| OBJ-4 | RF-06 | HU-6.1 | CA-6.1.1, CA-6.1.2 | Must | Propuesto |
| OBJ-5 Mantener consistencia de interfaz de usuario entre los sistemas de facturación | RF-12 | HU-12.1 | CA-12.1.1, CA-12.1.2 | Should | Propuesto |
...
| SUP-02 | El área de negocio corresponde a "Otras unidades de negocio" (Quinta María), sin mayor detalle adicional en el documento fuente. | Podría requerirse ajustar el alcance si existieran otras áreas involucradas. |
...
| PA-08 | ¿Cuál es el detalle campo por campo (mockup o especificación de pantalla) del acomodo de la pantalla de detalle de CFDI en Facturación Inmobiliaria que debe replicarse en Facturación Quinta María (RF-12)? | Negocio / Analista funcional |
